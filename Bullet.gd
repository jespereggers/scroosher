...
func _on_Bullet_body_entered(body):
	if not body.is_in_group("Player") and not "Bullet" in body.get_name():
		if not "Enemy" in body.get_name() and body.get_cellv(body.world_to_map($Pivot.get_global_position())) != -1:
			PlayAnimation()
			emit_signal("Collision", Data.Translator["GetName"][body.get_name()][str(body.get_cellv(body.world_to_map($Pivot.get_global_position())))], body, body.world_to_map($Pivot.get_global_position()))
			queue_free()
	...
	if "ByPlayer" in name and "Enemy" in body.get_name():
		...
		if get_parent().get_node("Space").has_node(body.get_name()):
			self.connect("EnemyGotShot",get_parent().get_node("Space").get_node(body.get_name()),"RenewHealth")
