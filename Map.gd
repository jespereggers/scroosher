...
func _ready():
	...
	Space.period = 10
	Space.persistence = 1
	Space.seed = int(rand_range(1,999))
...
func Asteroid(x,y):
	Position = $Asteroids.world_to_map(get_parent().get_node("Player").position)
	Position.x = int(Position.x-(x - WITH / 2))
	Position.y = int(Position.y-(y - HEIGHT / 2))
	return Position

func Planet(x,y):
	Position = $Planets.world_to_map(get_parent().get_node("Player").position)
	Position.x = int(Position.x-(x - WITH / 2))
	Position.y = int(Position.y-(y - HEIGHT / 2))
	return Position

func GenerateSpace(x,y):
	Digit = str(int(rand_range(0,3)))
	if randi()%1000+1 > 995:
		Planet = str(Planets[int(rand_range(0,3))])
...
func _on_Gun_DeleteTile(tile, node, pos):
	...
	LoadContext($Planets.world_to_map(get_global_mouse_position()))
	
	for loot in Data.Permissions[Data.Properties[tile]["Type"]]["Loot"]:
		Data.Inventory[loot] += int(rand_range(Data.Permissions[Data.Properties[tile]["Type"]]["Loot"][loot]["min"], Data.Permissions[Data.Properties[tile]["Type"]]["Loot"][loot]["max"]))
...
func LoadContext(CurrentPlanet):
	if $Planets.get_cellv(CurrentPlanet) != -1:
		...
		$Land.position.x = $Planets.map_to_world(CurrentPlanet).x + 16
		$Land.position.y = $Planets.map_to_world(CurrentPlanet).y + 16
